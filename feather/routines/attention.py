...
def _flash_attention_fp8_e4m3_acc_fp32_kernel(
    ...
    q_tile_b = _unpack_e4m3_to_fp16(tl.cast((q_tile_packed >> 8) & 0xFF, dtype=tl.uint16))
    q_tile_c = _unpack_e4m3_to_fp16(tl.cast((q_tile_packed >> 16) & 0xFF, dtype=tl.uint16))
    q_tile_d = _unpack_e4m3_to_fp16(tl.cast((q_tile_packed >> 24) & 0xFF, dtype=tl.uint16))
    ...
    for tile_k_idx in tl.range(arg1=0, arg2=_seq_len, step=_tile_sz):
        ...
        # load k
        k_tile_packed = tl.load(pointer=_k + (k_offsets), mask=mask_k[:, None], other=0.0)
        k_tile_a = _unpack_e4m3_to_fp16(tl.cast((k_tile_packed) & 0xFF, dtype=tl.uint16))
        k_tile_b = _unpack_e4m3_to_fp16(tl.cast((k_tile_packed >> 8) & 0xFF, dtype=tl.uint16))
        k_tile_c = _unpack_e4m3_to_fp16(tl.cast((k_tile_packed >> 16) & 0xFF, dtype=tl.uint16))
        k_tile_d = _unpack_e4m3_to_fp16(tl.cast((k_tile_packed >> 24) & 0xFF, dtype=tl.uint16))

        # load v
        v_tile_packed = tl.load(pointer=_v + (k_offsets), mask=mask_k[:, None], other=0.0)
        v_tile_a = _unpack_e4m3_to_fp16(tl.cast((v_tile_packed) & 0xFF, dtype=tl.uint16))
        v_tile_b = _unpack_e4m3_to_fp16(tl.cast((v_tile_packed >> 8) & 0xFF, dtype=tl.uint16))
        v_tile_c = _unpack_e4m3_to_fp16(tl.cast((v_tile_packed >> 16) & 0xFF, dtype=tl.uint16))
        v_tile_d = _unpack_e4m3_to_fp16(tl.cast((v_tile_packed >> 24) & 0xFF, dtype=tl.uint16))
...
def flash_attention_fp8_e4m3_acc_fp32_gpu(
    ...
    grid = lambda meta: (triton.cdiv(seq_len, meta['_tile_sz']), )
    out = torch.empty((seq_len, h_dim * 4), dtype=torch.float32, device="cuda")
    _flash_attention_fp8_e4m3_acc_fp32_kernel[grid](
        q, k, v, 
        out, 
        seq_len, 
        h_dim
