...
@triton.jit
def _relu_1d_fp8_ret_fp32_kernel(_x, _out, _n: tl.constexpr, _blk_size: tl.constexpr):
    ...
    tl.store(
        _out + _out_base + 0,
        tl.maximum(0.0, tl.cast(_x_a, dtype=tl.float16, bitcast=True).to(tl.float32)),
        mask=_mask,
    )
    tl.store(
        _out + _out_base + 1,
        tl.maximum(0.0, tl.cast(_x_b, dtype=tl.float16, bitcast=True).to(tl.float32)),
        mask=_mask,
    )
    tl.store(
        _out + _out_base + 2,
        tl.maximum(0.0, tl.cast(_x_c, dtype=tl.float16, bitcast=True).to(tl.float32)),
        mask=_mask,
    )
    tl.store(
        _out + _out_base + 3,
        tl.maximum(0.0, tl.cast(_x_d, dtype=tl.float16, bitcast=True).to(tl.float32)),
        mask=_mask,
    )


def relu1d_fp8_ret_fp32_feather_gpu(x: torch.Tensor, n: int):
